...
if (modalStartIndex !== -1 && modalEndIndex !== -1) {
    ...
    // Let's do some light CSS class renaming on quizLogic to match the new style
    quizLogic = quizLogic.replace(/style={{ borderBottom: '1px solid var\(--border-color\)', paddingBottom: '20px', marginBottom: '20px' }}/g, 'className="card" style={{ padding: "24px", marginBottom: "20px" }}');
    quizLogic = quizLogic.replace(/style={{ border: '1px solid var\(--border-color\)', padding: '15px', borderRadius: '6px', marginBottom: '15px' }}/g, 'className="card" style={{ padding: "20px", marginBottom: "15px" }}');
}

const newModalContent = `import React from 'react';
...
const CourseDetailsModal = (props) => {
    ...
    return (
        <div className="modal-overlay" style={{ padding: '20px', display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
          
          <div className="modal-content fade-in" style={{ maxWidth: '1000px', width: '100%', maxHeight: '90vh', padding: 0, overflow: 'hidden', display: 'flex', flexDirection: 'column', position: 'relative' }}>
            
            {/* FIXED CLOSE BUTTON */}
            <button 
              onClick={() => setSelectedCourse(null)}
              style={{ position: 'absolute', top: '15px', right: '20px', background: 'rgba(255,255,255,0.2)', border: 'none', color: 'white', padding: '8px', borderRadius: '50%', cursor: 'pointer', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center' }}
            >
              <X size={20} />
            </button>
            ...
            <div style={{ overflowY: 'auto', flex: 1 }}>
              ...
              {/* 1. HERO BANNER */}
              <div className="lms-hero" style={{ padding: '40px' }}>
              ...
              {/* 2. STICKY TABS */}
              <div className="lms-tabs" style={{ position: 'sticky', top: 0, zIndex: 90, background: 'white' }}>
                <button 
              ...
              {/* 3. CONTENT AREA */}
              <div className="lms-content" style={{ padding: '30px' }}>
                  ...
                  <div className="lms-grid-2 fade-in">
                    ...
                    <div>
                      <div className="card" style={{ padding: '30px', marginBottom: '20px' }}>
                        <h3 style={{ fontSize: '1.25rem', marginBottom: '16px', marginTop: 0 }}>About this Course</h3>
                      ...
                      <div className="card" style={{ padding: '30px' }}>
                        <h3 style={{ fontSize: '1.25rem', marginBottom: '16px', marginTop: 0 }}>Your Progress</h3>
                        ...
                        <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>Adaptive progress tracking will be enabled soon.</p>
                      </div>
                    </div>
                    ...
                    <div>
                      <div className="card" style={{ padding: '24px', marginBottom: '20px' }}>
                        <h3 style={{ fontSize: '1.1rem', marginBottom: '16px', marginTop: 0 }}>Instructor</h3>
                      ...
                      <div className="card" style={{ padding: '24px' }}>
                        <h3 style={{ fontSize: '1.1rem', marginBottom: '16px', marginTop: 0 }}>Class Roster ({selectedCourse.students.length})</h3>
                ...
                {activeTab === 'materials' && (
                  <div className="fade-in">
                    <div className="flex-between" style={{ marginBottom: '24px' }}>
                      <h3 style={{ fontSize: '1.25rem', margin: 0 }}>Course Modules</h3>
                    ...
                    {isLecturer && showCreateMaterialForm && (
                       <form onSubmit={(e) => handleCreateMaterial(e, selectedCourse._id)} className="card" style={{ padding: '24px', marginBottom: '30px', borderLeft: '4px solid var(--primary)' }}>
                         <h4 style={{ marginBottom: '15px', marginTop: 0 }}>Upload New Material</h4>
                          ...
                          <div key={mat._id} className="module-card fade-in">
                            <div className="module-header flex-between">
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                ...
                                {isLecturer && (
                                  <button className="btn btn-danger" style={{ padding: '4px 8px', fontSize: '0.75rem' }} onClick={() => handleDeleteMaterial(mat._id, selectedCourse._id)}>
                                    Delete
                                  </button>
                                )}
                              </div>
                    ...
                    {selectedAssignment ? (
                      <div className="fade-in">
                        ...
                        <div className="card" style={{ padding: '30px', marginBottom: '30px' }}>
                          ...
                          <p style={{ fontSize: '1rem', lineHeight: '1.6', margin: 0 }}>{selectedAssignment.description}</p>
                        </div>

                        {/* STUDENT SUBMISSION AREA */}
                        {isStudent && (
                          <div className="card" style={{ padding: '30px', borderTop: '4px solid var(--primary)' }}>
                            <h3 style={{ marginBottom: '20px', marginTop: 0 }}>Your Submission</h3>
                            {submissionLoading ? (
                              <p>Loading your submission...</p>
                            ) : studentSubmission ? (
                              <div style={{ background: 'var(--bg-primary)', padding: '20px', borderRadius: 'var(--radius)', border: '1px solid var(--border-color)' }}>
                                 <div className="flex-between" style={{ marginBottom: '16px' }}>
                                 ...
                                 <p style={{ whiteSpace: 'pre-wrap', margin: '0 0 20px 0' }}>{studentSubmission.content}</p>
                                 
                                 <div style={{ borderTop: '1px solid var(--border-color)', paddingTop: '20px' }}>
                                   <h4 style={{ fontSize: '1rem', marginBottom: '10px', marginTop: 0 }}>Feedback & Grade</h4>
                                   {studentSubmission.status === 'graded' ? (
                                     <div>
                                       ...
                                       <p style={{ margin: 0 }}><strong>Feedback:</strong> {studentSubmission.gradeFeedback || 'No additional feedback provided.'}</p>
                                     </div>
                                   ) : (
                                     <p style={{ color: 'var(--text-muted)', margin: 0 }}>Your assignment is currently under review by the instructor.</p>
                                   )}
                                 </div>
                              </div>
                            ) : (
                               ...
                               <div className="grid-cards" style={{ gridTemplateColumns: '1fr' }}>
                                 {submissions.map(sub => (
                                   <div key={sub._id} className="card" style={{ padding: '24px' }}>
                                     ...
                                     <div style={{ padding: '16px', background: 'var(--bg-primary)', borderRadius: 'var(--radius)', marginBottom: '16px', fontSize: '0.95rem', whiteSpace: 'pre-wrap', border: '1px solid var(--border-color)' }}>
                                       {sub.content}
                                     </div>
                      ...
                      <div>
                        <div className="flex-between" style={{ marginBottom: '24px' }}>
                          <h3 style={{ fontSize: '1.25rem', margin: 0 }}>Assignments</h3>
                          {isLecturer && (
                        ...
                        {isLecturer && showCreateAssignForm && (
                          <form onSubmit={(e) => handleCreateAssignment(e, selectedCourse._id)} className="card" style={{ padding: '24px', marginBottom: '30px', borderLeft: '4px solid var(--primary)' }}>
                            <h4 style={{ marginBottom: '15px', marginTop: 0 }}>New Assignment</h4>
                            <div className="form-group">
                              <label>Title</label>
...
console.log('Successfully wrote deeply fixed CourseDetailsModal.jsx');
